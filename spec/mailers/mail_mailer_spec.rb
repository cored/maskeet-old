...
module MailControllerTestHelper
  ...
  def deliver(action, mail_params = {}, send_params = {})
    MailMailer.dispatch_and_deliver(action, { :from => "no-reply@webapp.com", :to => "recepient@person.com" }.merge(mail_params), send_params)
    @delivery = last_delivered_mail
  end
end

describe MailMailer, "#notify_on_event email template" do
  ...
  it "includes welcome phrase in email text" do
    ...
    # MailMailer.dispatch_and_deliver(:notify_on_event, {}, { :name => "merb-mailer user" })
